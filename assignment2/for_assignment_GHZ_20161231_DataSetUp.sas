...
********************************
	...
	log on to wrds to get data -> Replace to WRDS macros for WRDS SAS Studio....
...
create table data
	  ...
	  and not missing(at)  and not missing(prcc_f) and not missing(ni) and datadate>='01JAN1957'd
...
============================================================;
					*======================GET CRSP IDENTIFIER=============================;
					proc sort data=crsp.ccmxpf_linktable out=lnk;
  					where LINKTYPE in ("LU", "LC", "LD", "LF", "LN", "LO", "LS", "LX") and
       					(2050 >= year(LINKDT) or LINKDT = .B) and (1950 <= year(LINKENDDT) or LINKENDDT = .E);
  					by GVKEY LINKDT; run;
					...
					*----------------------screen for only NYSE, AMEX, NASDAQ, and common stock-------------;
					...
					data temp;
						set temp;
					   	where exchcd in (1,2,3) and not missing(permno);
...
proc sql;
	...
	as select substr(compress(cusip),1,6) as cnum,c.gvkey,fyearq,fqtr,datadate,rdq,substr(sic,1,2) as sic2,
	  ...
	  where f.gvkey = c.gvkey
	  and f.indfmt='INDL' and f.datafmt='STD' and f.popsrc='D' and f.consol='C'
		and not missing(ibq) and datadate>='01JAN1957'd;
	quit;
						proc sort data=data nodupkey;
							by gvkey datadate;
							run;
